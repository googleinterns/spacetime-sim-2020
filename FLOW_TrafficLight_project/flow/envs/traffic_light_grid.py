...
class MyGridEnv(TrafficLightGridPOEnv):
    # travel time (trip duration), reward, demand(inflows)
    ...
    def observation_space(self):
        ...
        tl_box = Box(
            low=-np.inf,
            high=np.inf,
            shape=(3 * self.rows * self.cols + 4 + 4,), # hardcoded in
            dtype=np.float32)
        return tl_box
    ...
    def get_id_within_look_behind(self, outgoing=None):
        ...
        #     else:
        #         opposite_edge = outgoing
        #         # print(opposite_edge)
        #         ids_in_scope = filter(self.is_within_look_behind, self.k.vehicle.get_ids_by_edge(opposite_edge))
        #         for veh_id in list(ids_in_scope):
        #             self.k.vehicle.set_color(veh_id=veh_id, color=RED)
        #         return list(ids_in_scope)
        # else:
        #     return []
    ...
    def get_training_iter(self):
        # FIXME: check if ray instance is active, the run below, else return 0
        home_dir = os.path.expanduser('~')
